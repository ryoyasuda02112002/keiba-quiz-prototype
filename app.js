...
function home() {
  ...
  shell(`<section class="card hero"><span class="badge">身内テスト版</span><h2>今日のレースを選ぼう</h2><p>知っている馬から、まだ知らない名馬まで。あなたの競馬知識に合うコースを選択。</p><div class="mode-grid">${modes}</div>${existing ? '<button class="resume-button" id="resume">▶ 前回のクイズを再開する</button>' : ''}</section><section class="howto card"><p class="caption">HOW TO PLAY</p><h2>少ないヒントで、馬名を見抜け。</h2><ol><li><b>戦績</b>と騎手から、まずは一頭を絞り込む</li><li>迷ったらヒントを使う。使うほど得点は下がる</li><li>5問の合計スコアで、今日の自分に挑戦</li></ol><div class="definition-note"><p class="caption">DATA RULES</p><p><b>G1</b>：JRAがG1として実施する平地競走。<br><b>重賞</b>：JRAがG1・G2・G3として実施する平地競走。</p></div><p class="notice">データ訂正や感想は、共有者へお知らせください。</p></section>`);
