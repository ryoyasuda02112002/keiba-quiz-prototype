...
function home() {
  ...
  document.querySelector('#start').onclick = () => { attempt = existing ?? createAttempt(dateKey, setIds); save(); route = 'quiz'; render(); };
}
...
function complete(result) { const { answer } = getCurrent(); answer.result = result; answer.score = scoreQuestion({ correct: result === 'correct', ...answer }); answer.completedAt = new Date().toISOString(); save(); route = 'result'; render(); }
...
function result() {
  ...
  document.querySelector('#next').onclick = () => { if (attempt.currentPosition === 4) { attempt.completedAt = new Date().toISOString(); save(); route = 'summary'; } else { attempt.currentPosition += 1; save(); route = 'quiz'; } render(); };
}
