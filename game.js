// Generated by scripts/build-pages-bundle.js. Do not edit directly.
const HINTS = [
...
function scoreQuestion({ correct, revealedHints = [], usedFirstCharacter = false, wrongAnswerCount = 0 }) {
  if (!correct) return 0;
...
function createAttempt(setDate, questionIds) {
...
const totalScore = (attempt) => attempt.answers.reduce((sum, answer) => sum + (answer.score ?? 0), 0);
const correctCount = (attempt) => attempt.answers.filter((answer) => answer.result === 'correct').length;

const PREFIX = 'keiba-quiz:v1';
const key = (date) => `${PREFIX}:attempt:${date}`;
function loadAttempt(date) { try { const parsed = JSON.parse(localStorage.getItem(key(date)) ?? 'null'); return parsed?.version === 1 ? parsed : null; } catch { return null; } }
function saveAttempt(attempt) { localStorage.setItem(key(attempt.setDate), JSON.stringify(attempt)); localStorage.setItem(`${PREFIX}:last-result`, JSON.stringify(attempt)); }
function clearAttempt(date) { localStorage.removeItem(key(date)); }
...
render();
  ...
  if (target) target.textContent = '読み込みに失敗しました。ページを再読み込みしてください。';
});
